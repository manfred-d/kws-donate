    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.donations.update", [$donation->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label for="donor_id">{{ trans('cruds.donation.fields.donor') }}</label>
                            <select class="form-control select2" name="donor_id" id="donor_id">
                                @foreach($donors as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('donor_id') ? old('donor_id') : $donation->donor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('donor'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="currency" id="currency" required>
                                ...
                                @foreach(App\Models\Donation::CURRENCY_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('currency', $donation->currency) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="amount" id="amount" value="{{ old('amount', $donation->amount) }}" step="0.01" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="campaign_id" id="campaign_id" required>
                                @foreach($campaigns as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('campaign_id') ? old('campaign_id') : $donation->campaign->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\Models\Donation::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $donation->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="type" id="type" value="{{ old('type', $donation->type) }}">
                            @if($errors->has('type'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('type') }}
                                </div>
